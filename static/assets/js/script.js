        ...
        if (plugins.search.length) {
            ...
            for (i = 0; i < plugins.search.length; i++) {
                ...
                searchItem.submit($.proxy(function() {
                    $('<input />').attr('type', 'visible').attr('name', "filter").attr('value', this.filter).appendTo(this.element);
                    return true;
                }, options, this))
            }
        }
   ...
   */
    if (plugins.isotope.length) {
        ...
        for (i = 0; i < plugins.isotope.length; i++) {
            var isotopeItem = plugins.isotope[i], filterItems = $(isotopeItem).closest('.isotope-wrap').find('[data-isotope-filter]'), iso;
